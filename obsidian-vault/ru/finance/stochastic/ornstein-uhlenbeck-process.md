...
Симуляция OU-процесса ($\theta=2.0$, $\mu=0.5$, $\sigma=0.3$, $X_0=1.5$): большие отклонения экспоненциально возвращаются к $\mu=0.5$ с периодом полураспада $\approx 0.35$ года. Стационарная полоса $\mu \pm \sigma_\infty$ охватывает ~68% вероятности.
...
```chart
{
  ...
  "lines": [
    {"dataKey": "X", "stroke": "#3b82f6", "name": "Путь X(t)"},
    {"dataKey": "mean", "stroke": "#ef4444", "name": "Долгосрочное среднее μ"}
  ]
}
```
