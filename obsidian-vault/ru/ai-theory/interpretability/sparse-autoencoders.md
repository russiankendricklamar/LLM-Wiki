...
График показывает компромисс между качеством восстановления и разреженностью при масштабировании размера словаря: большие словари достигают меньшей MSE восстановления при том же L0-бюджете, иллюстрируя явление разбиения признаков.
...
```chart
{
  ...
  "lines": [
    {"dataKey": "recon_mse", "stroke": "#ef4444", "name": "MSE восстановления"},
    {"dataKey": "monosemantic_pct", "stroke": "#10b981", "name": "Моносемантические признаки (%)"}
  ]
}
```
...
**Процесс обучения.** SAE обучаются на больших коллекциях активаций замороженной [[llm|LLM]]:
...
def train_sae(model, activations, n_epochs=50, lr=2e-4, batch_size=512):
    """Обучение SAE на заранее собранных активациях [[llm|LLM]]."""
