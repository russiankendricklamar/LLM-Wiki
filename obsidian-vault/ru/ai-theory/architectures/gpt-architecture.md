---
title: GPT
category: AI Architectures
order: 130
lang: ru
slug: gpt-architecture
---

## Обзор
GPT (Generative Pre-trained [[transformer-architecture|Transformer]]) — это архитектура языковой модели, состоящая только из декодеров [[transformer-architecture|Transformer]], разработанная OpenAI.

## Механизм
GPT использует маскированное внимание (masked self-attention), гарантируя, что предсказание для позиции $i$ зависит только от токенов до $i$. Обучается на задаче каузального моделирования языка (предсказание следующего токена):
$$ L = \sum_i \log P(u_i \mid u_{i-k}, \dots, u_{i-1}) $$
Эта авторегрессионная структура делает её чрезвычайно эффективной для генерации текста.