...
Реальные кредитные сети содержат несколько типов узлов (корпоративный заёмщик, банк, суверен, гарант) и рёбер (кредит, облигация, акция, гарантия). [[transformer-architecture|Трансформер]] гетерогенных графов (HGT) назначает тип-специфичные проекции:
...
[[llm-financial-analysis]] — признаки, извлечённые [[llm|LLM]] из ковенантов и финансового текста, как атрибуты узлов GNN
