...
Modern schedulers can perform **chunked prefill**: they mix a chunk of prompt tokens from a new request with the decoding tokens of existing requests to perfectly balance the [[inference-serving|GPU]]'s compute and memory bandwidth on every single clock cycle.
...
gantt
    ...
    dateFormat  s
    axisFormat %S
    section [[inference-serving|GPU]] Slots
    Req A (Done)     :b1, 0, 2s
    Req D (New)      :b4, 2, 8s
    Req B (Done)     :b2, 0, 5s
    Req E (New)      :b5, 5, 8s
