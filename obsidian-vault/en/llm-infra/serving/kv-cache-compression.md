...
- **Multi-Head Latent [[attention-mechanisms|Attention]] (MLA)**: Introduced by **DeepSeek-V2**, this technique compresses KV vectors into a low-rank latent vector, drastically reducing the storage requirements while maintaining high performance.
...
- **H2O (Heavy Hitter Oracle)**: Keeps tokens that frequently receive high [[attention-mechanisms|attention]] scores ("heavy hitters") and the most recent tokens, discarding the rest.
