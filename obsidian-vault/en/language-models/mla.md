...
**Multi-Head Latent [[attention-mechanisms|Attention]] (MLA)** is a novel [[attention-mechanisms|attention]] mechanism introduced by **DeepSeek (V2/V3)** designed to solve the "KV-cache bottleneck." 

In standard Multi-Head [[attention-mechanisms|Attention]] (MHA), the memory required to store the KV-cache grows linearly with the number of heads and layers, becoming a massive cost during inference. Earlier solutions like **Grouped-Query [[attention-mechanisms|Attention]] (GQA)** reduced the cache size but sacrificed model capacity. MLA achieves **better compression than GQA** while maintaining (or exceeding) the performance of full MHA.
