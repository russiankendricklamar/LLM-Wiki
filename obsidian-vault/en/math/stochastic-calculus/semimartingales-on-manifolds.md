...
When transforming coordinates on a [[manifold-learning|manifold]] $M$, Itô integrals lack the standard chain rule due to the second-order term. Stratonovich integrals, denoted by $\circ dX_t$, obey classical calculus rules, making them geometrically intrinsic:
$$ f(X_t) - f(X_0) = \int_0^t \nabla f(X_s) \circ dX_s $$
A semimartingale $X$ on $M$ is typically driven by a Stratonovich [[stochastic-differential-equations|SDE]] $dX_t = \sum_{i=1}^m V_i(X_t) \circ dW_t^i + V_0(X_t) dt$.

## Eells-Elworthy-Malliavin Construction

Brownian motion on a Riemannian [[manifold-learning|manifold]] $(M, g)$ cannot be directly defined via independent increments. Instead, it is constructed using the orthonormal frame bundle $O(M)$. We solve a canonical [[stochastic-differential-equations|SDE]] for the frame $U_t$:
