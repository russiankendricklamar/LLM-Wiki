...
**Maximum-[[shannon-entropy|entropy]] principle:** among all distributions on $\mathbb{R}^k$ with fixed $\boldsymbol{\mu},\boldsymbol{\Sigma}$, the [[shannon-entropy|entropy]] maximiser is exactly the Gaussian. This is why MaxEnt returns a Gaussian whenever the constraints fix only the first two moments.
